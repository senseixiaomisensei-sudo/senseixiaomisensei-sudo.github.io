...
const source = await readFile(new URL("functions/rvc/[[path]].js", root), "utf8");
...
test("Pages RVC relay uses the Worker service binding and supplies same-origin GET origin", async () => {
  ...
  const response = await onRequest({
    request: new Request("https://postprep-ae6.pages.dev/rvc/status"),
    env: {
      POSTPREP_GATEWAY: {
        async fetch(request) {
          forwarded = request;
...
test("Pages RVC relay preserves a GitHub Pages browser origin", async () => {
  let origin;
  const response = await onRequest({
    request: new Request("https://postprep-ae6.pages.dev/rvc/models", {
