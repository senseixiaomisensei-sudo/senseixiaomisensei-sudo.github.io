...
def finalize_true_peak_safe(output_wav: Path, ceiling_dbfs: float = -1.5) -> bool:
    """Universal last-step safety for EVERY conversion output.

    Sample-peak limiters cannot see intersample peaks: a file whose samples
    peak at -0.45 dBFS can still overshoot 0 dBFS between samples and clip on
    playback (the reported distortion on loud masters). This measures the
    4x-oversampled true peak and, when it exceeds the ceiling, rescales the
    whole file with a uniform (inaudible) gain. Works for every mode and
    every kind of source audio.
    """
    try:
        measure = subprocess.Popen(
            [
                "ffmpeg", "-nostdin", "-v", "error", "-i", str(output_wav),
                "-af", "aresample=176400:resampler=soxr", "-f", "f64le", "-",
            ],
            stdout=subprocess.PIPE, stderr=subprocess.DEVNULL,
        )
        true_peak = 0.0
        while True:
            chunk = measure.stdout.read(8 * 1024 * 1024)
            if not chunk:
                break
            block = np.frombuffer(chunk, dtype="<f8")
            if block.size:
                true_peak = max(true_peak, float(np.max(np.abs(block))))
        measure.stdout.close()
        measure.wait(timeout=120)
        true_peak_db = 20.0 * math.log10(max(true_peak, 1e-9))
        if true_peak_db <= ceiling_dbfs:
            return False
        gain_db = ceiling_dbfs - true_peak_db
        scaled = output_wav.with_name(output_wav.stem + "-tpsafe" + output_wav.suffix)
        result = subprocess.run(
            [
                "ffmpeg", "-nostdin", "-v", "error", "-y", "-i", str(output_wav),
                "-af", "volume=" + f"{gain_db:.4f}dB",
                "-c:a", "pcm_s16le", str(scaled),
            ],
            check=False, capture_output=True,
...
async def process_conversion_job(
    *,
    ...
    try:
        ...
        await asyncio.to_thread(finalize_true_peak_safe, output_wav, -1.5)
