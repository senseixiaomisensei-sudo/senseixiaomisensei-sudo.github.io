(() => {
  ...
  const translations = {
    zh: {
      ...
      privacy: {
        ...
        servicesBody: "页面样式、图标和首页图片都使用仓库内置资源。你主动使用云端生成、深度处理、Skills 筛选、公开 GitHub Skill 权限护照或授权声音生成时，浏览器会向 Cloudflare Turnstile 请求一次性验证令牌。Skills 筛选与公开 Skill 读取由服务端查询 GitHub 公开 API；为减少公开 API 限流，公开候选或 Skill 元数据可在服务端内存中短时缓存，缓存键不含草稿内容。声音生成在部署启用后由单独的受保护 GPU 服务处理，不会向浏览器公开 GPU 地址或密钥。若启用 AI 排序或深度解释，也只在你主动点击后传递当前文本。Google AdSense 自动广告脚本会在公开页面异步加载。",
        turnstilePolicy: "查看 Cloudflare Turnstile 隐私说明",
        googleAdsPolicy: "查看 Google 广告与 Cookie 说明",
        adsTitle: "广告与第三方代码",
        adsBody: "公开页面使用 Google AdSense 自动广告。Google 及其广告技术合作伙伴可能请求设备信息、IP 地址，使用 Cookie 或本地存储，并根据你的地区、同意状态和广告设置投放个性化、非个性化或受限广告。广告异步加载，不会读取 PostPrep 输入框中的草稿或音频内容。广告是否展示及展示位置由 Google、页面内容和可用广告共同决定。",
        controlTitle: "你的选择",
        controlBody: "你可以随时清空输入框、关闭页面或清除浏览器的本地站点数据。语言偏好保存在当前浏览器的 localStorage 中；Google 广告相关选择由 AdSense 的隐私与消息机制及你的浏览器设置管理。",
        back: "返回首页",
      },
      platform: {
        xiaohongshu: "小红书",
    ...
    en: {
      ...
      home: {
        ...
        rvcTitle: "AI Voice Changer",
        rvcBody: "Pick a character voice, upload or record your own audio, and convert it with RVC in one click.",
        open: "Open",
        ...
        noteThree: "Google AdSense Auto ads load asynchronously",
      },
      length: {
        eyebrow: "TEXT LENGTH CHECKER",
      ...
      privacy: {
        ...
        servicesBody: "Page styles, icons, and the homepage photo are bundled with this site. When you choose cloud generation, deep processing, Skills discovery, a public GitHub Skill passport, or authorized voice generation, the browser requests a one-time verification token from Cloudflare Turnstile. Skills discovery and public Skill reading query GitHub's public API server-side; public candidate or Skill metadata may be kept briefly in server memory to reduce rate limits, and cache keys do not include draft content. When deployed, voice generation is processed by a separate protected GPU service; neither its address nor its token is exposed to the browser. If AI ranking or deep explanation is enabled, the current text is sent only after you choose that action. Google AdSense Auto ads load asynchronously on public pages.",
        turnstilePolicy: "View Cloudflare's Turnstile privacy notice",
        googleAdsPolicy: "View Google's advertising and cookie information",
        adsTitle: "Advertising and third-party code",
        adsBody: "Public pages use Google AdSense Auto ads. Google and its advertising technology partners may request device information and IP addresses, use cookies or local storage, and serve personalized, non-personalized, or limited ads based on region, consent, and account settings. Ads load asynchronously and do not read drafts or audio from PostPrep input fields. Google, page content, and available inventory determine whether and where ads appear.",
        controlTitle: "Your control",
        controlBody: "You can clear any input, close the page, or erase local site data at any time. Your language preference is stored in this browser's localStorage; Google ad choices are managed through AdSense Privacy & messaging and your browser settings.",
        back: "Back to home",
      },
      platform: {
  ...
  function renderSharedLayout() {
    ...
    const links = [
      ...
      { id: "rvc", href: "rvc.html", label: "shared.rvc" },
  ...
  function initCommon() {
    ...
    if (menuToggle && mobileNavigation) {
      const closeMenu = () => {
        ...
        menuToggle.setAttribute("aria-label", t("shared.menu"));
      ...
      document.addEventListener("pointerdown", (event) => {
        if (!menuToggle.contains(event.target) && !mobileNavigation.contains(event.target)) closeMenu();
      });
      window.addEventListener("resize", () => {
        if (innerWidth >= (document.body.dataset.page === "rvc" ? 1400 : 1200)) closeMenu();
      }, { passive: true });
    }
